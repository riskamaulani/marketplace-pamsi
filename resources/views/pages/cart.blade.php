...
<div class="space-y-5">
    ...
    <section x-data="{
        ...
        <div class="lg:sticky lg:top-4 p-4 mx-auto w-full max-w-96 h-fit space-y-3 bg-white rounded-lg shadow">
            ...
            <div>
                <a
                    :href="sumTotal() != 0 ? '{{ route('checkout') }}?state=' + btoa(JSON.stringify(selectedProducts)) : '#'"
                    class="mt-2 w-full inline-flex justify-center items-center px-4 py-2 border border-transparent rounded-md font-semibold text-xs text-white tracking-widest transition ease-in-out duration-150"
                    :class="sumTotal() != 0 
            ? 'bg-myaccent hover:bg-myaccenthover active:bg-myaccent' 
            : 'bg-gray-500 cursor-not-allowed'">
                    <span class="text-sm text-white font-semibold">Checkout</span>
                </a>
            </div>
